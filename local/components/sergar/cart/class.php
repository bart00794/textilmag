...
class SergarCartComponent extends CBitrixComponent
{
    ...
    protected function getSum()
    {
        ...
        foreach ($this->arResult['ITEMS'] as $arItem) {
            $totalCost += $arItem['PRICE_TOTAL'];
        }
        $this->arResult['SUBTOTAL'] = $totalCost;
        if ($totalCost < 10000) {
            $this->arResult['DELIVERY'] = '250';
        }
        $this->arResult['TOTAL'] = $this->arResult['SUBTOTAL'] + $this->arResult['DELIVERY'];
    }
    ...
    protected function formSubmit($request)
    {
        ...
        $arResult['TEXT'] = '<table border="1" cellpadding="0" cellspacing="0" >
						   <tr>
							 <th style="width: 50px;" align=center >' . Loc::getMessage(
                "SERGAR_COMPONENT_CART_CLASS_TABLE_NUMBER"
            ) . '</th>
							 <th style="width: 300px;" align=center>' . Loc::getMessage(
                "SERGAR_COMPONENT_CART_CLASS_TABLE_NAME"
            ) . '</th>
							 <th style="width: 90px;" align=center>' . Loc::getMessage(
                "SERGAR_COMPONENT_CART_CLASS_TABLE_SIZE"
            ) . '</th>
							 <th style="width: 90px;" align=center>' . Loc::getMessage(
                "SERGAR_COMPONENT_CART_CLASS_TABLE_PRICE"
            ) . '</th>
							 <th style="width: 80px;" align=center>' . Loc::getMessage(
                "SERGAR_COMPONENT_CART_CLASS_TABLE_COUNT"
            ) . '</th>
							 <th style="width: 90px;" align=center>' . Loc::getMessage(
                "SERGAR_COMPONENT_CART_CLASS_TABLE_PRICE_SUMMARY"
            ) . '</th>
						   </tr>  ';
        $number = 1;
        $price = 0;
...
//            dump($arItem);
            $arResult['TEXT'] .= '<tr>
							 ...
							 <td align=center>' . number_format($arItem['PRICE'], 2, '.', ' ') . ' ' . Loc::getMessage(
                    "SERGAR_COMPONENT_CART_CLASS_RUBBLE"
                ) . '</td>
                    ...
                    '.',
                    ' '
                ) . ' ' . Loc::getMessage("SERGAR_COMPONENT_CART_CLASS_RUBBLE") . '</td>
						   </tr>';
            $number++;
        ...
        $arResult['TEXT'] .= '
			   <tr>
				<td colspan="4" style="text-align: left;" ><span style="margin-left: 10px;" ><strong>' . Loc::getMessage(
                "SERGAR_COMPONENT_CART_CLASS_TABLE_PRICE_TOTAL"
            ) . ':</strong></span></td>
				<td  align=center><strong>' . $number . '</strong></td>
				<td  align=center><strong>' . number_format($price, 2, '.', ' ') . ' ' . Loc::getMessage(
                "SERGAR_COMPONENT_CART_CLASS_RUBBLE"
            ) . '</strong></td>
			   </tr></table>';
