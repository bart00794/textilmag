...
$ShowNewOfferName="ShowNewOffer".$intSubIBlockID.$ID.$arSubCatalog['SKU_PROPERTY_ID'];
$CheckProductNameName="CheckProductName".$intSubIBlockID.$ID.$arSubCatalog['SKU_PROPERTY_ID'];
...
$sTableID = "tbl_iblock_sub_element_".md5($strSubIBlockType.".".$intSubIBlockID.$intSubIBlockID.$ID.$arSubCatalog['SKU_PROPERTY_ID']);
    ...
    if ($lAdmin->EditAction())
    {
        ...
        if ($boolSubCatalog)
        {
            if ($boolCatalogPrice && (isset($_POST["CATALOG_PRICE"]) || isset($_POST["CATALOG_CURRENCY"])))
            {
                ...
                {
                    $arCatalogGroup['ID'] = intval($arCatalogGroup['ID']);
                ...
                foreach ($CATALOG_PRICE as $elID => $arPrice)
                {
                    ...
                    if (!empty($arCatalogGroupList))
                    {
                        ...
                        {
                            $arPrice[$arCatalogGroup["ID"]] = doubleval($arPrice[$arCatalogGroup["ID"]]);
                            if ($arPrice[$arCatalogGroup["ID"]] != doubleval($CATALOG_PRICE_old[$elID][$arCatalogGroup["ID"]])
...
if (!(false == B_ADMIN_SUBELEMENTS_LIST && $bCopyS))
{
    $wf_status_id = "";
    ...
    while ($arRes = $rsData->NavNext(true, "f_"))
    {
        ...
        $edit_url = $UrlMeEdit.'?WF=Y&type='.urlencode($strSubIBlockType).'&IBLOCK_ID='.$intSubIBlockID.'&lang='.LANGUAGE_ID.'&PRODUCT_ID='.$ID.'&ID='.$arRes_orig['ID'].'&TMP_ID='.$strSubTMP_ID.$sThisSectionUrl.'&by='.$by.'&order='.$order.'&SKU_PROPERTY_ID='.$arSubCatalog['SKU_PROPERTY_ID'];
        $arRows[$arRes["ID"]] = $row = $lAdmin->AddRow($arRes["ID"], $arRes, $edit_url, GetMessage("IB_SE_L_EDIT_ELEMENT"), true);

        $boolEditPrice = false;
        $boolEditPrice = CIBlockElementRights::UserHasRightTo($intSubIBlockID, $arRes["ID"], "element_edit_price");
        ...
        foreach ($arSelectedProps as $aProp)
        {
            ...
            $table_id = md5($f_ID.':'.$aProp['ID'].$intSubIBlockID.$ID.$arSubCatalog['SKU_PROPERTY_ID']);
            if ($aProp["MULTIPLE"] == "Y")
            {
...
if (!empty($arRows))
{
    ...
    {
        $edit_url = $UrlMeEdit.'?WF=Y&type='.urlencode($strSubIBlockType).'&IBLOCK_ID='.$intSubIBlockID.'&lang='.LANGUAGE_ID.'&PRODUCT_ID='.$ID.'&ID='.$row->arRes['orig']['ID'].'&TMP_ID='.$strSubTMP_ID.$sThisSectionUrl.'&by='.$by.'&order='.$order.'&SKU_PROPERTY_ID='.$arSubCatalog['SKU_PROPERTY_ID'];
            ...
            else
            {
                ...
                )
                {
                    ...
                    $arActions[] = array(
                        ...
                        "ACTION"=>"(new BX.CAdminDialog({
                            'content_url': '".$UrlMeEdit."?WF=Y&type=".CUtil::JSEscape(htmlspecialcharsbx($strSubIBlockType))."&IBLOCK_ID=".$intSubIBlockID."&lang=".LANGUAGE_ID."&PRODUCT_ID=".$ID."&ID=".$row->arRes['orig']['ID']."&TMP_ID=".urlencode($strSubTMP_ID).$sThisSectionUrl.'&by='.$by.'&order='.$order.'&'.bitrix_sessid_get()."&SKU_PROPERTY_ID=".$arSubCatalog['SKU_PROPERTY_ID']."',
                ...
                )
                {
                    $arActions[] = array(
                        ...
                        "ACTION"=>"(new BX.CAdminDialog({
                            'content_url': '".$UrlMeEdit."?WF=Y&type=".CUtil::JSEscape(htmlspecialcharsbx($strSubIBlockType))."&IBLOCK_ID=".$intSubIBlockID."&lang=".LANGUAGE_ID."&PRODUCT_ID=".$ID."&ID=".$row->arRes['orig']['ID']."&TMP_ID=".urlencode($strSubTMP_ID).'&by='.$by.'&order='.$order."&action=copy".$sThisSectionUrl.'&'.bitrix_sessid_get()."&SKU_PROPERTY_ID=".$arSubCatalog['SKU_PROPERTY_ID']."',
                            'content_post': '".(!(defined('BX_PUBLIC_MODE') && BX_PUBLIC_MODE == 1) ? '&bxsku=Y' : '')."&bxpublic=Y',
                            'draggable': true,
                            'resizable': true,
                            'buttons': [BX.CAdminDialog.btnSave, BX.CAdminDialog.btnCancel]
...
function <?=$CheckProductNameName?>(id)
{
    if (!id)
        return false;
    var obj = BX(id);
...
function <?=$ShowNewOfferName?>(id)
{
    var mxProductName = <?=$CheckProductNameName?>(id);
    if (!mxProductName)
    ...
    else
    {
        ...
        (new BX.CAdminDialog({
            'content_url': '<?=$UrlMeEdit?>?WF=Y&type=<? echo CUtil::JSEscape(htmlspecialcharsbx($strSubIBlockType)); ?>&IBLOCK_ID=<? echo $intSubIBlockID; ?>&lang=<? echo LANGUAGE_ID; ?>&PRODUCT_ID=<? echo $intSubPropValue; ?>&ID=0&TMP_ID=<? echo urlencode($strSubTMP_ID).$sThisSectionUrl; ?>&by=<?=$by?>&order=<?=$order?>&SKU_PROPERTY_ID=<?=$arSubCatalog['SKU_PROPERTY_ID']?>',
    ...
    if ($boolIBlockElementAdd)
    {
        $aContext[] = array(
            ...
            "LINK"=>"javascript:".$ShowNewOfferName."('btn_sub_new')",
   ...
   /* if ($boolIBlockElementAdd)
	...
	$tbl = CUtil::JSEscape($lAdmin->table_id);
	$aUserOpt = CUserOptions::GetOption("global", "settings");
	
		echo '
		<script type="text/javascript">

		function ReloadOffers'.$intSubIBlockID.$ID.$arSubCatalog['SKU_PROPERTY_ID'].'()
		{
			'.$lAdmin->ActionAjaxReload($lAdmin->GetListUrl(true)).'
		}
		</script>
		';
		if($_REQUEST["mode"]=='list'||$_REQUEST["mode"]=='frame')
		{
			echo '
			<script type="text/javascript">
				'.$lAdmin->table_id.'.list_url="'.$lAdmin->GetListUrl(true).'";
			</script>
			';
		}
    ...
    //$lAdmin->DisplayList(B_ADMIN_SUBELEMENTS_LIST);
		...
		echo '
		<script type="text/javascript">
		var '.$lAdmin->table_id.'= new BX.adminSubList("'.$tbl.'", {context_ctrl: '.($aUserOpt["context_ctrl"] == "Y"? "true":"false").'}, "'.$lAdmin->GetListUrl(true).'");
		</script>
		';

        $menu = new CAdminPopup($lAdmin->table_id."_menu", $lAdmin->table_id."_menu",false,array('zIndex' => 4000));
        $menu->Show();


        echo '<div id="'.$lAdmin->table_id.'_result_div">';
        $lAdmin->Display();
        echo '</div>';
