...
$SKU_PROPERTY_ID=(int)$_REQUEST["SKU_PROPERTY_ID"];
...
if (array_key_exists('bxsku', $_REQUEST) && 'Y' == $_REQUEST['bxsku'])
	$BX_SUB_SETTINGS=true;
else
	$BX_SUB_SETTINGS=false;
define("MODULE_ID", "iblock");
define("ENTITY", "CIBlockDocument");
define("DOCUMENT_TYPE", "iblock_".$IBLOCK_ID);
define("BX_SUB_SETTINGS",$BX_SUB_SETTINGS);
...
if (array_key_exists('PRODUCT_NAME', $_REQUEST) && '' != trim($_REQUEST['PRODUCT_NAME']))
{
...
$bSubCopy = false;
if (array_key_exists('action', $_REQUEST) && 'copy' == $_REQUEST['action'])
	$bSubCopy = true;
	...
	)
	{
		...
		else
		{
			...
			if ($bBizproc)
			{
                ...
                top.ReloadOffers<?=$IBLOCK_ID.$intProductID.$SKU_PROPERTY_ID?>();
                </script><?
                die();
            }
        }
...
<input type="hidden" name="SKU_PROPERTY_ID" value="<?=$SKU_PROPERTY_ID?>">
<?
$tabControl->EndEpilogContent();

$customTabber->SetErrorState($bVarsFromForm);
$tabControl->AddTabs($customTabber);
$strFormAction = $UrlMeEdit."?type=".urlencode($type)."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$IBLOCK_ID."&find_section_section=".intval($find_section_section);
